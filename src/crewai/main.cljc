(ns crewai.main
  "Kotodama WASM entrypoint for the Crewai clean-room actor (L4) — Clojure port.
  ...
  facts (`crewai.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "crewai")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"crewai.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "crewai-compat" :tier tier :entities entities} 200])
...
(defn start! [] :crewai-compat/ready)
